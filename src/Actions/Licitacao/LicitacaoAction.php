...
class LicitacaoAction extends Action
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response, string $codigoOrgao)
    {
        $params = $this->validate($request->getQueryParams());
        $resultado = $this->licitacaoService->findBetweenDates($params['data_inicial'], $params['data_final'], $codigoOrgao);
